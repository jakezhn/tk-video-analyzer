...
async def run_analysis_pipeline(job_id: str, video_url: str):
    ...
    try:
        ...
        # Step 2: Detect scenes and extract keyframes
        with open(status_file, "w") as f:
            f.write("detecting scenes")
        keyframes_paths = await asyncio.to_thread(_detect_scenes_and_extract_keyframes, video_path, job_dir)

        # Step 3: Transcribe Audio
        with open(status_file, "w") as f:
            f.write("transcribing audio")
        transcript_text = await asyncio.to_thread(_transcribe_audio, video_path, job_dir)

        # Step 4: Generate Report with LLM
        with open(status_file, "w") as f:
            f.write("generating report")
        report_content = await _generate_report_with_llm(transcript_text, keyframes_paths)
...
def _detect_scenes_and_extract_keyframes(video_path: str, job_dir: str) -> list[str]:
...
async def _generate_report_with_llm(transcript: str, keyframe_paths: list[str]) -> str:
    ...
    prompt_parts = [
        ...
        "The final report will have two parts: the speech transcript in English, and all other analysis in Simplified Chinese.",
        "\n---",
...
def _transcribe_audio(video_path: str, job_dir: str) -> str:
    """Extracts audio from video, saves it, and returns the transcript."""
    ...
    try:
        ...
        (
            ...
            .run("/opt/homebrew/bin/ffmpeg", capture_stdout=True, capture_stderr=True, overwrite_output=True)
        )
    except ffmpeg.Error as e:
        print("ffmpeg-python error:")
    ...
    with open(os.path.join(job_dir, "transcript.txt"), "w") as f:
        f.write(transcript)
